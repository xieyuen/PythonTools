...
#             console.logger.warning('命令为空，自动转 exit')
#             ConsoleCmd.exit_(None)
#         action = self.command_map.get(command)
#         if action:
#             action(command[global_vars['temp']['cmd']['space']['index'][0]:])
#         else:
#             console.logger.warning(f'函数 {command} 未定义！')
#         return True
...
class ConsoleCmd:
    ...
    def help(__other_cmd: str):
        ...
        if __other_cmd.isspace() or __other_cmd == '':
            ...
            if __other_cmd in global_vars['help']:
                ...
                console.logger.warning(f'未找到 {__other_cmd } 的帮助！')
        return None
    ...
    def rmdir(__other_cmd: str):
        ...
        try:
            ...
            console.logger.catch_exc('好像触发了什么奇奇怪怪的异常？')
        return False
    ...
    def version(__none=None) -> None:
        console.logger.info(f"版本号: {global_vars['console']['version']}")
...
def main() -> None:
    ...
    while True:
        ...
        if __action:
            ...
            console.logger.warning(f'Command "{input_str}" not found')
